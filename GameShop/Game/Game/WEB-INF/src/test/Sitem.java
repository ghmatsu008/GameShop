	...
	public class Sitem  extends HttpServlet {
				...
				public void doGet( HttpServletRequest request, 
			        ...
			        ItemData id=null;
					GetItem it = new GetItem();
					List<ItemData> itemList = it.getItem();
					
			        
					String tmp=null;
					StringBuffer sel = new StringBuffer();
					sel.append("<OPTION value=\"0\" selected>選んでください</OPTION>");
					for ( Iterator<ItemData> ite = itemList.iterator(); ite.hasNext();) {
						
						id = (ItemData)ite.next(); // <-- コレクション内移動
						tmp = "<OPTION value=\""+id.getId()+"\" >"+id.getNam()+"</OPTION>";
						sel.append( tmp ); 
